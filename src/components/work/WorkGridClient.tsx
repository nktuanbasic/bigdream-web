      ...
      {sortedProjects.map((project, i) => {
        ...
        return (
          <motion.div key={project.id} variants={itemVariants} className={`flex flex-col group ${colSpanClass}`}>
            <Link href={`/work/${project.id}`} className="relative block overflow-hidden rounded-sm bg-[#111] w-full h-full">
              <div className={`${aspectClass} relative w-full h-full overflow-hidden`}>
                <div 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent group-hover:from-black/70 transition-colors duration-700"></div>
                
                {/* Typography đưa vào TRONG ảnh theo đúng sketch */}
                <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 flex flex-col xl:flex-row justify-between items-start xl:items-end gap-4 z-10">
